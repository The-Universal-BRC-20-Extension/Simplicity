...
SKIPPED: crypto module API changed (is_op_return_script, extract_op_return_data
moved/removed; address validation behavior differs). Rewrite in Phase B.
"""

import pytest

pytestmark = pytest.mark.skip(reason="crypto API changed; rewrite in Phase B")
...
def test_extract_address_from_script_p2wsh():
    ...
    addr = crypto.extract_address_from_script(script_hex)
    assert addr.startswith("bc1q")  # P2WPKH addresses start with bc1q
