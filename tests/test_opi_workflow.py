...
def test_opi_workflow():
    ...
    try:
        ...
        # Test mock validator
        mock_validator = Mock()
        mock_validator.get_deploy_record.return_value = Mock(
            ticker="TEST",
...
def test_processor_return_type():
    ...
    try:
        ...
        tx = {
            "txid": "test_tx",
            "vout": [{"scriptPubKey": {"hex": "76a914..."}}],
            "vin": []
        }
        
        result, objects, commands = processor.process_transaction(
            tx, 1000, 0, 1609459200, "test_hash"
        )
