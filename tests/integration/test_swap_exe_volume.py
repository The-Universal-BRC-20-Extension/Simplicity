...
def test_swap_exe_volume_100_positions(db_session):
    """
    Test swap.exe can fill multiple positions (100) in one execution.
    Executor pays SRC, receives DST; position owners receive SRC.
    Assertions tolerate AMM fees and fill order.
    """
    # Setup deploys
    deploy_src = Deploy(
        ticker="SRC",
    ...
    # Executor SRC reduced (AMM + fees may use slightly different amount)
    ...
    # Executor receives DST (AMM output; fees apply)
    executor_dst = db_session.query(Balance).filter_by(address="executor", ticker="DST").first()
    assert executor_dst is not None
    assert executor_dst.balance >= Decimal("100000000")  # At least ~0.1 BTC

    # At least some positions closed (fixture lp_dst may absorb some fills)
...
def test_swap_exe_volume_1000_positions(db_session):
    """
    Stress test: swap.exe with 1000 positions (1 BTC each).
    Validates bulk fill capacity; assertions tolerate AMM behavior.
    """
    # Setup deploys
    deploy_src = Deploy(
        ticker="SRC",
...
def test_swap_exe_volume_large_amounts_thousands_btc(db_session):
    """
    Test swap.exe with large amounts (5000 BTC single position).
    Validates large-amount handling; assertions tolerate AMM fees.
    """
    # Setup deploys
    deploy_src = Deploy(
        ticker="SRC",
    ...
    executor_dst = db_session.query(Balance).filter_by(address="executor", ticker="DST").first()
    assert executor_dst is not None
    assert executor_dst.balance >= Decimal("100000000")  # At least 1 BTC received (AMM/slippage varies)


def test_swap_exe_volume_concurrent_executions(db_session):
    """
    Test multiple sequential swap.exe executions; each fills positions.
    Each executor swaps 1 BTC; validates multi-tx flow.
    """
    # Setup
    deploy_src = Deploy(
        ticker="SRC",
    ...
    def get_executor(tx):
        try:
            idx = int(str(tx.get("txid", "")).split("_")[-1])
            return executors[min(idx, 9)]
        except (ValueError, IndexError):
            return executors[0]

    processor.get_first_input_address = MagicMock(side_effect=get_executor)

    # Execute 10 swap.exe transactions
    for i in range(10):
